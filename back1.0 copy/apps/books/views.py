...
try:
    ...
    AdvancedPDFProcessor = None  # 如果依赖未安装，设为None
    ...
    def _optimize_chapter_boundaries(self, chapters, pages_data):
        ...
        for chapter in chapters:
            ...
            # 2. 单层阿拉伯数字序号格式：1. ...（仅包含一位阿拉伯数字加英文句点）
                ...
                chapter['type'] = 'main'
                match_result = re.match(r'^(\d+)\.\s+(.+)$', title)
                chapter['number'] = match_result.group(1)
                chapter['full_number'] = chapter['number']
                chapter['title'] = f"{chapter['full_number']}. {match_result.group(2)}"
            # 3. 二级标题格式：1.1、1.2 等
    ...
    def import_pdf(self, request):
        ...
        try:
            ...
            for main_chapter in chapters:
                ...
                # 创建主章节
                start_page = main_chapter['start_page'] + 1
                end_page = main_chapter['end_page'] + 1
                created_main_chapter = Chapter.objects.create(
                    ...
                    description=f'由PDF自动生成，包含第{start_page}-{end_page}页内容',
