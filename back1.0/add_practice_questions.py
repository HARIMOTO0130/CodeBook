...
def add_practice_questions():
    """为每个章节的练习题集添加至少5道不同类型的练习题"""
    ...
    for practice in practices:
        try:
            ...
            choice_question = {
                ...
                "type": "choice",
                "title": "选择题",
                "question": get_choice_question(book, chapter),
                "options": [
                    {"id": 1, "content": get_choice_option(book, chapter, 1), "is_correct": False},
                    {"id": 2, "content": get_choice_option(book, chapter, 2), "is_correct": True},
                    {"id": 3, "content": get_choice_option(book, chapter, 3), "is_correct": False},
                    {"id": 4, "content": get_choice_option(book, chapter, 4), "is_correct": False}
                ],
                "difficulty": 1,
                "order": 1
            }
            ...
            fill_question = {
                ...
                "type": "fill",
                "title": "填空题",
                "question": get_fill_question(book, chapter),
                "blanks": [
                    {"id": 1, "correct_answer": get_fill_answer(book, chapter, 1), "placeholder": "第一空"},
                    {"id": 2, "correct_answer": get_fill_answer(book, chapter, 2), "placeholder": "第二空"}
                ],
                "difficulty": 2,
                "order": 2
            }
            ...
            code_completion_question = {
                ...
                "title": "代码补全题",
                "question": get_code_completion_question(book, chapter),
                "code_template": get_code_template(book, chapter, "completion"),
                "test_cases": get_test_cases(book, chapter, "completion"),
                "difficulty": 2,
                "order": 4
            }
            ...
            programming_question = {
                ...
                "type": "programming",
                "title": "编程题",
                "question": get_programming_question(book, chapter),
                "code_template": get_code_template(book, chapter, "programming"),
                "test_cases": get_test_cases(book, chapter, "programming"),
                "difficulty": 3,
                "order": 5
            }
            ...
            print(f"✅ 成功为练习题集添加了 {len(questions)} 道题目")
...
def get_choice_question(book, chapter):
    """根据书籍和章节生成选择题题目"""
    ...
    questions = {
        # 大学计算机基础与应用
        (1, 1): f"在《{book_title}》中，计算机的核心处理器是以下哪一项？",
        (1, 2): f"在《{book_title}》中，操作系统的主要功能不包括以下哪一项？",
        (1, 3): f"在《{book_title}》中，Excel中用于求和的函数是？",
        
        # 数据分析与可视化入门
        (2, 1): f"在《{book_title}》中，以下哪个不是常用的数据分析工具？",
        (2, 2): f"在《{book_title}》中，数据清洗的主要目的是？",
        (2, 3): f"在《{book_title}》中，以下哪种图表适合展示数据的分布情况？",
        
        # 人工智能与机器学习基础
        (3, 1): f"在《{book_title}》中，以下哪项不是人工智能的应用领域？",
        (3, 2): f"在《{book_title}》中，监督学习的特点是？",
        (3, 3): f"在《{book_title}》中，神经网络的基本组成单元是？",
    }
    
    return questions.get((book.id, chapter.id), f"在《{book_title}》中，关于{chapter_title}的选择题")
...
def get_test_cases(book, chapter, question_type):
    ...
    test_cases = {
        ...
        (2, 1, "completion"): [
            {"id": 1, "input": {}, "expected_output": "DataFrame with data"}
        ],
        (2, 1, "programming"): [
            {"id": 1, "input": {"file": "data.csv"}, "expected_output": "DataFrame head"}
        ],
    }
    
    return test_cases.get((book.id, chapter.id, question_type), [
        {"id": 1, "input": {}, "expected_output": "expected result"}
    ])
