...
export default {
  ...
  methods: {
    ...
    viewResource(resource) {
      const fullUrl = this.getFullResourceUrl(resource.fileUrl);
      // 图片资源直接在新窗口查看，其他资源下载
    ...
    downloadResource(resource) {
      ...
      // 使用getFullResourceUrl函数构建正确的URL
      const fullUrl = this.getFullResourceUrl(resource.fileUrl);
      ...
      // 方法2: 使用fetch API下载（备用方案）
      // 这种方法可以处理更复杂的情况，如需要认证的情况
      /*
      fetch(fullUrl)
        .then(response => {
          if (!response.ok) {
    ...
    moreOptions(resource) {
      const fullUrl = this.getFullResourceUrl(resource.fileUrl);
      const resourceDetails = `资源名称: ${resource.name}
资源类型: ${this.getTypeName(resource.type)}
资源大小: ${this.formatSize(resource.size)}
上传时间: ${this.formatDate(resource.uploadTime)}
浏览次数: ${resource.views}
...
文件链接: ${fullUrl}`;
    ...
    async loadResources() {
      try {
        ...
        this.resources = resourcesList.map(item => ({
          ...
          classId: '', // 教学资源不属于特定班级
          fileUrl: item.file || '' // 添加文件URL
        }))
        
        // 确保分页数据正确
        this.currentPage = 1
        // totalPages是计算属性，会自动根据filteredResources和pageSize计算，不需要手动赋值
    ...
    getFullResourceUrl(fileUrl) {
      console.log('Original fileUrl:', fileUrl);
      
      // 如果fileUrl已经是完整的URL，则直接返回
      if (fileUrl && typeof fileUrl === 'string') {
        if (fileUrl.startsWith('http://') || fileUrl.startsWith('https://')) {
          console.log('Returning full URL directly:', fileUrl);
          return fileUrl;
        }
      }
      ...
      // 直接使用文件名构建URL，使用正确的媒体文件路径
      const fileName = fileUrl.split('/').pop();
      const fullUrl = `http://localhost:8000/media/teaching_resources/${fileName}`;
